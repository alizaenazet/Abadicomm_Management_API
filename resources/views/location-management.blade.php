...
<div class="min-h-screen bg-gray-100">
  <div class="max-w-6xl mx-auto p-8">
    <div class="mb-8">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-3xl font-bold text-gray-900 mb-2">Location Management</h1>
          <p class="text-gray-600">Manage all locations in the system</p>
        </div>
        <button
          onclick="showAddModal()"
          class="flex items-center gap-2 px-6 py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg transition font-semibold shadow-md"
        >
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          ...
          <tr class="hover:bg-gray-50 transition">
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $location->schedules_count > 0 ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                {{ $location->schedules_count }} schedule(s)
              </span>
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
              <button
                onclick="showEditModal({{ $location->id }}, '{{ addslashes($location->name) }}')"
              ...
              <button
                onclick="showDeleteModal({{ $location->id }}, '{{ addslashes($location->name) }}', {{ $location->schedules_count }})"
          ...
          <tr>
            <td colspan="3" class="px-6 py-12 text-center text-gray-500">
              ...
              <p class="text-sm mt-1">Click "Add New Location" to create one</p>
            </td>
          </tr>
          @endforelse
        </tbody>
    ...
    <div class="bg-white rounded-xl p-6 w-96 shadow-2xl">
      ...
      <form action="{{ route('location.store') }}" method="POST">
        ...
        <input
          ...
          class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-green-600 focus:outline-none mb-4"
          required
        />
  ...
  <div id="editModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-xl p-6 w-96 shadow-2xl">
      <h3 class="text-xl font-bold text-gray-900 mb-4">Edit Location</h3>
  ...
  <div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-xl p-6 w-96 shadow-2xl">
      <h3 class="text-xl font-bold text-gray-900 mb-4">Delete Location</h3>
  ...
  <script>
    ...
    function showEditModal(id, name) {
      document.getElementById('editForm').action = `/location/${id}`;
    ...
    function showDeleteModal(id, name, scheduleCount) {
      if (scheduleCount > 0) {
        document.getElementById('deleteMessage').innerHTML =
          `<strong class="text-red-600">Warning!</strong> This location "<strong>${name}</strong>" is being used in ${scheduleCount} schedule(s). You cannot delete it until all related schedules are removed.`;
      ...
      document.getElementById('deleteForm').action = `/location/${id}`;
